...
def inspect_boot_image(filepath: Path):
    ...
    if file_size < 16384:
        print(f"Error: Image size ({file_size} bytes) is too small to be a valid Android boot image.")
        return False
    ...
    # Format: magic(8s), kernel_size(I), kernel_addr(I), ramdisk_size(I), ramdisk_addr(I),
    # second_size(I), second_addr(I), tags_addr(I), page_size(I), header_version(I), os_version(I)
    try:
        ...
        print(f"Kernel Size:      {kernel_size} bytes (Addr: 0x{kernel_addr:08x})")
        print(f"Ramdisk Size:     {ramdisk_size} bytes (Addr: 0x{ramdisk_addr:08x})")
        print(f"Second Size:      {second_size} bytes (Addr: 0x{second_addr:08x})")
